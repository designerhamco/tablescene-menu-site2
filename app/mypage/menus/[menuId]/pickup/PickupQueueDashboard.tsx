    ...
    <div className="space-y-6">
      ...
      <section className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm md:p-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            <h3 className="type-content-title">수동 대기번호 등록</h3>
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="text-sm font-bold text-zinc-600 underline decoration-zinc-300 underline-offset-4 hover:text-zinc-950"
          >
            고객 대기판 열기
          </a>
        </div>
        <form action={createAction} className="mt-5 flex max-w-md gap-2">
          ...
          <button
            ...
            className="rounded-2xl bg-zinc-950 px-5 py-3 text-sm font-bold text-white disabled:cursor-not-allowed disabled:bg-zinc-300"
          >
      ...
      <section className="rounded-3xl border border-zinc-200 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-zinc-100 px-5 py-4 md:px-6">
          <div>
            <h3 className="type-content-title">오늘 운영 현황</h3>
      ...
      <section className="rounded-3xl border border-zinc-200 bg-white shadow-sm">
        <div className="border-b border-zinc-100 px-5 py-4 md:px-6">
          <h3 className="type-content-title">오늘 처리 이력</h3>
        </div>
        {completedEntries.length > 0 ? (
          <ul className="divide-y divide-zinc-100">
            {completedEntries.map((entry) => (
              <li key={entry.id} className="flex items-center justify-between gap-4 px-5 py-4 md:px-6">
                <span className="text-lg font-bold">{entry.queueNumber}</span>
                <span className="text-xs font-bold text-zinc-400">{STATUS_LABELS[entry.status]}</span>
              </li>
            ))}
          </ul>
        ) : (
    ...
    <li className="px-5 py-4 md:px-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4">
          <span className="min-w-14 text-3xl font-bold tabular-nums">{entry.queueNumber}</span>
        ...
        <form action={action} className="flex gap-2">
          ...
          <button
            ...
            value="cancelled"
            disabled={pending}
            className="rounded-xl border border-zinc-200 px-4 py-2 text-xs font-bold text-zinc-500 disabled:opacity-40"
          >
            취소
          </button>
          <button
            ...
            value={nextStatus}
            disabled={pending}
            className="rounded-xl bg-zinc-950 px-4 py-2 text-xs font-bold text-white disabled:bg-zinc-300"
          >
