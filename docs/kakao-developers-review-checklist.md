...
This checklist is for MenuLink Kakao Developers privacy consent item review.

## Service signup evidence

- Submit `/sign-up` as the signup flow URL.
- Capture the top of `/sign-up` where the `메뉴링크 회원가입` title and `카카오로 시작하기` button are visible.
...
- App name: use `메뉴링크` or `MenuLink`.
- App icon: use the MenuLink service logo.
...
- App description: describe the MenuLink digital menu/service management product.
...
- If MenuLink does not actually store or use CI, review whether CI should remain a required Kakao consent item before resubmission.
