...
- 2026-09-16 썸머 블루 4개 페이지를 가격 열 보정까지 반영한 최신 코드로 1920×1080 재캡처했다. 자동 fit·overflow·console 검사를 통과했고, 사용자가 베이커리 할인 제거를 최종 조건으로 지정한 뒤 PR #216에서 이를 반영해 최종 육안 승인을 완료했다.
...
- 2026-09-16 썸머 블루 기본 할인은 바질 크림 라떼의 `시즌할인`·`썸머 시그니처 특별가`, 카푸치노의 `커피할인`·`클래식 커피 특별가` 두 종류만 사용한다. 클래식 버터 스콘은 할인 대상에서 제거해 정상가만 표시한다.
