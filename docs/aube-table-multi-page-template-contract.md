...
- 편집 화면의 `카테고리명 글자` 설정은 오브 테이블에서 페이지명과 코스명을 하나의 제목 체계로 함께 변경한다. `가게명 글자`와 `나머지 글자` 역할도 같은 저장 구조와 webfont loader를 사용하며 모바일 헤더·언어 UI·스마트호출 bottom sheet까지 기본 Pretendard/Tenor Sans fallback을 상속한다.
