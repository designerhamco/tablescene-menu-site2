...
- 페이지 제목 크기는 기존 비율을 유지한다. 그 아래 코스명·메뉴명·가격·설명은 태블릿을 기준으로 `clamp()` 유동 스케일을 사용해 820px 세로부터 1180px 가로까지 자연스럽게 확대하며, 고정 픽셀 단계로 급격히 작아지지 않는다.
- 태블릿·데스크톱의 메뉴명은 22~30px, 가격은 18~24px, 설명은 16~20px 범위로 맞춘다. 코스와 메뉴 항목 사이의 여백도 같은 방식으로 유동 조절해 2열에서도 읽기 밀도와 가격선 정렬을 유지한다.
...
- 태블릿 가로 1180×820과 세로 820×1180에서 메뉴명·가격·설명·코스 간격의 비율과 2열 가격선 줄바꿈을 확인한다.
