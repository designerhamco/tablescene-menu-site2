...
보관 종료 예정일까지 복구할 수 있습니다. 복구 가능 기간은 무료체험 종료 후 30일, 유료 구독 종료 후 90일, 결제 실패·미납 상태에서는 30일을 기준으로 합니다.
...
- 기본 제목: `[아티메뉴] 데이터 보관 기간 종료 예정 안내`
- 당일 제목: `[아티메뉴] 데이터 보관 기간이 오늘 종료 예정입니다`
- 본문:
...
- 제목: `[아티메뉴] 이메일 고지 발송 테스트`
- 본문:

```text
아티메뉴 이메일 고지 발송 설정이 정상적으로 연결되었는지 확인하기 위한 테스트 메일입니다.
```
...
- `EMAIL_FROM`: 예) `아티메뉴 <admin@dndcommerce.co.kr>`
