...
- `DONE` 미리보기 기기 의미·타이포 연동 3차 보정 — 1180px 태블릿 가로가 desktop breakpoint를 지나더라도 명시적 `previewDevice=tablet` 규칙을 사용한다. 최신 기준은 단일페이지 역할 글자에 1.12 장치 배율을 더하고 안전 맞춤 엔진으로 잘림을 방지한다. 선데이 라인은 카테고리 과대 확대 없이 상품명/설명 위계를 유지하고 모카 포레스트 디저트는 4개로 조정했다.
...
- `DONE` 판매 템플릿 디자인 2차 보정 — 단일페이지 4종은 스타터 밀도·4개 언어·유동 타이포·간격·모카 포레스트 언어 UI를 보정했고, 모바일 언어 UI를 제목 위 독립 행 오른쪽으로 통일했다. 태블릿은 공통 1.12 장치 글자 배율을 사용하고 방향은 한 개의 화면 회전 아이콘으로 전환한다. 모카 카테고리는 1.08배, 대표 역할은 PC·태블릿 1.25배·모바일 1배로 보정했으며 선데이 상·하단 행 간격과 라운드 포커스 1:1:1 열 비율을 확정했다. 멀티페이지 2종은 기존 승인 디자인과 독립 반응형·모션 계약을 유지했다. Display는 미리보기 안내·하단 제어·HOT/ICE 가격 열·할인 대상을 보정했으며, 베이커리 할인 제거를 최종 조건으로 한 썸머 블루 육안 승인을 완료
