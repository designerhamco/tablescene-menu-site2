...
- `DONE` 판매 템플릿 디자인 2차 보정 — 단일페이지 4종은 스타터 밀도·4개 언어·유동 타이포·간격·모카 포레스트 언어 UI를 보정했고, 모바일 언어 UI를 제목 위 독립 행 오른쪽으로 통일했다. 태블릿 가로는 PC보다 작은 역할별 글자 비율과 중앙 정렬된 2행 기기·방향 툴바를 사용한다. 멀티페이지 2종은 기존 승인 디자인과 독립 반응형·모션 계약을 유지했다. Display는 미리보기 안내·하단 제어·HOT/ICE 가격 열·할인 대상을 보정했으며, 베이커리 할인 제거를 최종 조건으로 한 썸머 블루 육안 승인을 완료
