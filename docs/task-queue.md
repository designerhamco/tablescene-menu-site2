...
- `DONE` 미리보기 상단 UI 정리 — 브라우저 안내의 Guest는 제거하고 URL 입력창처럼 보이는 긴 둥근 사각형과 회색 채움 원형 프로필·더보기·확대/축소를 유지. 단일·멀티페이지의 기기 선택기는 기본 펼침이며, 화살표를 누르면 동일한 448px 폭·54px 높이의 바가 위로 이동해 얇은 손잡이만 남고 다시 누르면 아래로 열린다. 마우스 근접 효과와 별도 짧은 pill은 사용하지 않음
...
- `DONE` 단일페이지 4종 유동 디자인 — 핵심 글자·간격을 고정 px가 아닌 viewport·container 기반 유동값으로 전환하고 PC·태블릿·모바일 시각 회귀 검증. 모카 포레스트 언어 전환 UI는 흰색 계열로 보정
- `DONE` 선데이 라인 타이포 위계 보정 — 화면 채움 중에도 카테고리명 > 메뉴명/가격 > 설명의 비율을 유지하고 상단 좌우 문구를 메뉴 설명과 같은 계산 배율로 연결. 로컬 실제 렌더링에서 22.38px > 15.27px > 11.26px, 상단 11.24px를 검증
- `DONE` 디자인 엔진 최종 자동 회귀 — 공개·판매 템플릿 31개 PC·모바일 화면의 HTTP·빈 화면·overflow·이미지·요청·console/page error·axe critical/serious와 미리보기 행동 검사를 유지. 최종 DOM overflow 시 글자·간격 안전 축소 후 재검증하는 종단 보정을 추가하고, 활성 7개 템플릿의 비한국어 21개 route와 툴바·선데이 라인 브라우저 검사를 실패 0건으로 고정
