...
- `DONE` 미리보기 기기 의미·타이포 연동 3차 보정 — 1180px 태블릿 가로가 desktop breakpoint를 지나더라도 명시적 `previewDevice=tablet`로 작은 타이포·여백 규칙을 사용한다. 단일페이지 4종의 PC 1440×900과 태블릿 1180×820을 직접 비교해 모두 `ready`·`overflow=false`·가로 넘침 0px과 태블릿 글자 축소를 확인했다. 대표 상품명은 메뉴 상품명과 실제 계산 크기까지 동일하게 고정했고, 선데이 라인은 카테고리 과대 확대 없이 상품명/설명 위계를 회복했다. 모카 포레스트 디저트는 4개로 조정
- `DONE` 단일페이지 역할별 타이포그래피 완전 연동 — 디스플레이·멀티페이지를 제외한 4종에서 `상품명=대표 상품명`, `텍스트칩=대표 텍스트칩`, `상품 설명=대표 설명=카테고리 설명=가게 설명=안내 1/2/3`, `상품 가격=대표 가격`을 각각 하나의 유동 역할 토큰으로 연결했다. 대표 가격의 별도 확대 배율과 굵기를 제거하고 가격 기준을 container 폭이 아닌 viewport 기준으로 계산해 위치가 달라도 실제 크기가 같게 했다. PC·태블릿·모바일 12개 화면에서 크기·서체·굵기·행간·자간·스타일을 계산값으로 비교하며 불일치 시 실패하도록 브라우저 QA를 고정했다.
...
- `DONE` 판매 템플릿 디자인 2차 보정 — 단일페이지 4종은 스타터 밀도·4개 언어·유동 타이포·간격·모카 포레스트 언어 UI를 보정했고, 모바일 언어 UI를 제목 위 독립 행 오른쪽으로 통일했다. 태블릿 가로는 PC보다 작은 역할별 글자 비율을 사용하고 방향은 한 개의 화면 회전 아이콘으로 전환한다. 모카 카테고리는 1.08배, 대표 역할은 일반 메뉴 역할의 1.25배로 보정했으며 선데이 상·하단 행 간격과 라운드 포커스 1:1:1 열 비율을 확정했다. 멀티페이지 2종은 기존 승인 디자인과 독립 반응형·모션 계약을 유지했다. Display는 미리보기 안내·하단 제어·HOT/ICE 가격 열·할인 대상을 보정했으며, 베이커리 할인 제거를 최종 조건으로 한 썸머 블루 육안 승인을 완료
