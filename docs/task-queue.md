...
- `DONE` 단일페이지 역할별 타이포그래피 완전 연동 — 디스플레이·멀티페이지를 제외한 4종에서 `상품명=대표 상품명`, `텍스트칩=대표 텍스트칩`, `상품 설명=대표 설명=카테고리 설명=가게 설명=안내 1/2/3`, `상품 가격=대표 가격`을 각각 하나의 유동 역할 토큰으로 연결했다. 대표 영역도 모든 기기에서 대응 메뉴 역할과 같은 계산 크기를 사용한다. 보조언어명은 모든 스타터 항목에 필수이며 공통 보조서체의 600 굵기를 사용한다. PC·태블릿·모바일 화면에서 크기·서체·굵기·행간·자간·스타일을 계산값으로 비교하며 불일치 시 실패하도록 브라우저 QA를 고정했다.
