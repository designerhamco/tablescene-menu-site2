...
- `IN_PROGRESS` 판매 가능한 멀티페이지 다이닝 `오브 테이블` — 커버 선택 노출, 최대 10개 메뉴 페이지, 1·2열/정렬 설정, 코스·직접 메뉴 혼합, 페이지 탭 드래그 정렬, 고정 dot 이동과 공개 renderer를 구현. 템플릿명과 스타터 이름을 통일하고 기본 커버는 `THE MENU`·한글 설명으로 구성했으며 기본정보 로고가 있을 때만 표시하고 텍스트 매장명은 커버에서 반복하지 않는다. 시그니처·단품·드링크 완성형 스타터와 전용 커버 이미지를 연결. 모든 페이지의 제목·설명은 중앙 정렬한다. `Signature Course`는 코스·메뉴 정보까지 중앙 정렬하고 `A La Carte Menu`·`Drink Menu` 본문은 왼쪽 정렬한다. 참고 시안에 맞춰 Pretendard/Tenor Sans, 문장형 영문 대소문자, PC 기준 페이지 제목 최대 68px·코스명 26~32px·메뉴명 17~20px·가격 15~18px·설명 13~15px 유동 스케일과 최대 1440px 본문 폭·유동 여백을 적용했다. 커버 제목은 최대 112px의 베이지 골드로 유지하고 카피 사이 간격을 축소했다. 커버 기본색은 네이비 `#0D172A`, 메뉴 배경과 하단 fade는 순백색이다. 페이지 제목·설명, 헤더·코스, 코스·메뉴, 메뉴 아이템 사이의 간격을 태블릿 중심으로 압축했다. 가격 미노출 코스의 실선은 가격 열 끝까지 확장하고, 가격 미노출 메뉴는 연결 점선도 숨기며, 노출 점선은 0.65px 반경·5px 간격의 옅은 점으로 표시한다. 5.5px 배경 없는 dot, 전용 SVG와 hover 모션을 사용한 PC 전용 `Prev`·`Next`를 추가했다. 터치·마우스 스와이프와 버튼·dot 이동은 같은 방향성 페이지 전환 모션을 사용하고 세로 스크롤·모션 감소 설정을 보존한다. 커버 이미지에는 선택 배경색을 기본 75%로 겹치고 편집 화면에서 0~100%로 조절하도록 연결. additive migration과 generated types 갱신은 2026-08-30 사용자 승인 아래 Production 완료. pilot 지정·판매 노출은 별도 승인 필요
...
`오브 테이블` additive migration의 Production 적용과 최종 시각 승인을 마치기 전까지 신규 판매 노출, 스마트호출 Production runtime과 `매장 운영` 허브는 fail-closed 상태를 유지한다. 실제 Owner의 메뉴 가져오기 E2E는 기존 내용을 교체해도 되는 전용 draft가 지정된 뒤 수행한다. QR오더·PG·주문 기능은 장기 비활성 제품으로 보존하며 별도의 재개 결정 전에는 구현·활성화하지 않는다.
