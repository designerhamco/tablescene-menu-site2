...
- `DONE` 디스플레이 `썸머 블루` 할인 안내 중복 제거 — 기본 할인가와 `오픈할인` 텍스트칩은 유지하고 같은 의미의 `오픈 기념 한정 할인` 보조 문구를 제거해 항목별 할인 신호를 하나로 정리
- `DONE` 디스플레이 `썸머 블루` 최신 최종 육안 승인 — 2026-09-16 사용자가 미리보기 UX·HOT/ICE 가격 열 보정 화면을 승인하고 할인 문구 중복 제거만 최종 조건으로 지정. 해당 조건 반영 후 자동 fit·overflow·console·브라우저 회귀 검사를 재통과
