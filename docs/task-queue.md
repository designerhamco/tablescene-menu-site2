...
`오브 테이블`과 `메종 마레`는 같은 9,900원 멀티페이지 상품의 신규 구매·교체 후보로 운영한다. 스마트호출 실제 write는 단일 QA 메뉴판 allowlist를 유지해 판매 노출과 분리한다. QR오더·PG·주문 기능은 장기 비활성 제품으로 보존하며 별도 재개 결정 전에는 구현·활성화하지 않는다. `썸머 블루` 사용자 육안 승인, AI 상담 Production 공개, 멀티페이지 A·B 판매 전환, 나머지 단일페이지 최종 시각 점검은 2026-09-14 완료했다. 다음 운영 우선순위는 이력이 없는 전용 QA 계정의 실제 결제수단 등록·30일 무료체험·첫 메뉴 6크레딧·해지 예약/복구 E2E, 실제 모바일 메일 클라이언트 시각 점검, 외부 공급자 비밀키의 계획된 교체 순이다. 실제 카드 입력·비밀키 발급 및 폐기처럼 사용자의 공급자 권한이 필요한 단계만 별도 운영 절차로 남긴다.
