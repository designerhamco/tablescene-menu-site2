...
- `TODO` 마이페이지 메뉴판 카드의 QR 진입을 전부 `QR 관리`로 통일 — 모든 메뉴판에 대표 주소·QR 복사·재다운로드를 공통 제공하고, 테이블별 QR은 스마트호출 지원 상품에서만 추가 노출하도록 대표 QR과 테이블 runtime 권한을 분리
