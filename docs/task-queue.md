...
- `IN_PROGRESS` 판매 가능한 멀티페이지 다이닝 `오브 테이블` — 커버 선택 노출, 최대 10개 메뉴 페이지, 1·2열/정렬 설정, 코스·직접 메뉴 혼합, 페이지 탭 드래그 정렬, 고정 dot 이동과 공개 renderer를 구현하고 로컬 QA 완료. additive migration은 Draft PR에서만 대기하며 Production 적용·판매 노출은 별도 승인 필요
