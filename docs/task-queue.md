...
- `IN_PROGRESS` 전체 회귀 QA — 공개 홈·신청·고객지원·인증·약관과 판매 템플릿을 PC 1440×900·모바일 390×844로 반복 검사하는 `npm run qa:public-surfaces`를 추가했다. 2026-09-16 Production 31개 화면에서 HTTP 오류·가로 overflow·깨진 이미지·console/page error·유효한 같은 도메인 요청 실패가 모두 0임을 확인했다. 다음은 인증 계정의 구매·메뉴 생성·편집·다국어·AI·QR·호출·대기번호·직원 권한과 접근성·실제 모바일 메일 클라이언트 검증, `docs/runbooks/public-surface-regression-qa.md`
