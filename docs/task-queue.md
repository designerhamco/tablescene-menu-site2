...
- `IN_PROGRESS` 사이트 전체 시각 시스템 재정비 — 1차로 Pretendard 기반 전역 타이포 토큰과 의미별 클래스(디스플레이·페이지·섹션·하위 섹션·본문 제목·3단계 본문·레이블·내비게이션·캡션)를 확정하고, 홈페이지 공통 섹션·헤더·푸터·AI 상담과 일반 페이지 H1/H2에 적용했다. 새 일반 페이지 제목이 시스템을 벗어나면 실패하는 계약 테스트도 추가했다. 고객 편집형 메뉴 템플릿·공개 메뉴·대기번호판은 별도 타이포 시스템으로 유지한다. 다음은 색상·여백·그리드·버튼·입력·카드·모달·토스트·반응형 규칙을 공개 사이트 → 인증 → MY/메뉴판·편집기·매장 운영 순으로 작은 PR에 나눠 개편
