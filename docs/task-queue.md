...
- `DONE` 멀티페이지 B타입 탐색 기반 — A타입의 커버·메뉴 페이지·코스·직접 메뉴·편집·번역·스마트호출 계약과 schema를 재사용하고, PC·태블릿은 왼쪽 페이지 메뉴/오른쪽 콘텐츠, 모바일은 헤더 아래 가로 스와이프 페이지명 탭으로 분리. B타입에서는 하단 dot·Prev/Next·페이지 swipe를 제거하고 1440×900·1180×820·390×844 production build route QA와 A타입 회귀를 완료. 실제 생성·구매·교체 후보에는 노출하지 않는 `coming_soon` 상태
