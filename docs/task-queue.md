...
- `IN_PROGRESS` 고객센터 전용 디자인 — FAQ·AI 상담·1:1 문의의 1차 정보 구조와 무그림자 표면 체계는 완료. 중복된 FAQ 진입 카드와 하단 검정 CTA를 제거하고, 고객센터의 AI 상담 카드는 별도 페이지 이동 없이 공통 퀵 상담창을 직접 열도록 연결했다. AI 상담 모달의 중복 머리말을 제거하고 모바일에서 추천 질문·대화·동의·입력 영역이 한 화면에 더 자연스럽게 쌓이도록 밀도를 조정했으며 상시 1:1 문의 전환 링크를 제공한다. FAQ 검색과 문의 내역 화면의 2차 보정은 계속 진행
