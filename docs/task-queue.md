...
- `DONE` 단일페이지 언어 UI와 대표 영역 편집 계약 정리 — 오브 커피·모카 포레스트 PC·태블릿의 언어 변경 UI를 가게명 위 독립 행으로 이동했다. 대표 영역은 이미지 노출과 상품 정보 노출을 분리해 상품 연결 없이 이미지만 보여줄 수 있고, 상품 정보가 꺼져 있거나 연결 메뉴가 숨겨져도 이미지는 유지한다.
