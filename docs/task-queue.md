...
- `IN_PROGRESS` 사이트 전체 시각 시스템 재정비 — `DONE` Pretendard 기반 전역 타이포 단계: 기초 크기·굵기 토큰과 의미별 역할(디스플레이·페이지·섹션·하위 섹션·본문 제목·항목 제목·3단계 본문·레이블·내비게이션·캡션)을 확정했다. 홈페이지·공개 페이지·인증·고객센터·신청·MY/메뉴판·편집기·매장 운영·관리자와 중첩 컴포넌트의 H1~H6를 전수 연결하고, 일반 UI에서 900 굵기와 숫자 직접 입력 크기를 제거했다. 모든 이름 기반 본문 크기도 기초 토큰에 연결했으며 새 활성 UI가 계약을 벗어나면 실패하도록 `app` 라우트와 루트 `components` 전수 검사로 확장했다. `DONE` 버튼·카드 단계: 검정 주요 버튼, 흰색 보조 버튼, 빨강 위험 버튼과 40/44/52px 높이, 무확대 호버, 포커스·비활성 규칙을 공통화했다. 카드는 흰 배경·1px 중립선·24px 모서리·무그림자로 통일하고 MY/메뉴판, 편집기, 매장 운영, 고객센터, 인증, 신청 화면의 공통 액션과 표면을 연결했다. 기존 개별 클래스도 동일 토큰으로 보이도록 호환 계층을 두되 고객 편집형 메뉴 템플릿·공개 메뉴·대기번호판과 축소 시각 프리뷰는 별도 시스템으로 유지한다. 다음은 입력·선택·체크박스, 모달·토스트, 색상·여백·그리드·반응형 규칙을 순차 개편
...
- `IN_PROGRESS` AI 상담 콘텐츠·대화 UX 보정 — 현재 판매 정책·QR·AI 크레딧 지식과 추천 질문은 최신화. 출시 회귀 단계에서 실제 질문 세트의 장문·중복·잘못된 단정·1:1 문의 전환과 모바일 채팅창을 추가 검증
