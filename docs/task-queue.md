...
- `TODO` 현재 Production 기능 베이스라인 재검증 — 격리된 QA 메뉴판으로 스마트호출 접수·상태 전환·2분 제한, Display 대기번호 등록·픽업·수령, 대표·테이블별 QR 복사·다운로드·교체 후 기존 주소 폐기를 다시 확인하고 테스트 기록을 정리
- `TODO` 메뉴 제작 AI 실사용 E2E — QA 메뉴판에서 메뉴명·설명 생성, 메뉴 정리, 다국어 번역, 결과 저장·공개 반영, 실패 롤백, 크레딧 차감을 실제 API로 검증. 첫 메뉴 6크레딧 평생 1회 지급과 추가 메뉴 미지급도 신규 QA 계정에서 같이 확인
- `NEEDS_HUMAN` 신규 결제·30일 무료체험 실제 E2E — 사업자 확인·실제 결제수단 등록, 0원 시작, 해지 예약·복구, 30일 종료 경계의 첫 결제·만료 우선순위를 확인. 실제 카드 입력 단계만 사용자 작업 필요
- `TODO` 직원 권한 2차 설계 — 현재 고정 역할형 `매니저`·`편집자`·`주문 직원`·`조회 전용`을 역할 프리셋 + 직원별 허용/차단 덮어쓰기로 확장. 결제·구독·보관·삭제·직원 관리는 Owner 전용으로 잠그고, 제한 메뉴는 숨기지 않고 비활성 사유를 표시. Order 장기 비활성 정책에 맞춰 `주문 직원`은 `운영 직원`으로 재정의 검토
- `TODO` 사이트 전체 시각 시스템 재정비 — 색상·타이포·여백·그리드·버튼·입력·카드·모달·토스트·반응형 규칙을 먼저 토큰과 공통 컴포넌트로 확정한 뒤, 공개 사이트 → 인증 → MY/메뉴판·편집기·매장 운영 순으로 작은 PR에 나눠 개편
...
권장 순서는 ① 현재 Production의 호출·대기번호·QR·메뉴 AI 기준선 재검증 ② 실제 결제수단이 필요한 30일 무료체험·웰컴 크레딧·해지 전체 E2E ③ 역할 프리셋 + 직원별 권한 덮어쓰기 계약 확정 ④ 사이트 공통 디자인 시스템 확정 ⑤ 공개 사이트·인증·MY/메뉴판·편집기·매장 운영의 단계적 시각 개편 ⑥ 고객센터 디자인과 AI 상담 콘텐츠·대화 UX 보정 ⑦ 템플릿별 디자인 2차 보정 ⑧ 전체 기기·권한·외부 연동 출시 회귀 QA ⑨ 모바일 메일 확인·계획된 비밀키 교체이다. `오브 테이블`·`메종 마레`는 9,900원 멀티페이지 상품으로 유지하고, QR오더·PG·POS 주문은 별도 재개 결정 전까지 장기 비활성으로 보존한다. Production 실제 카드 입력·비밀키 발급·폐기 단계만 사용자 작업이 필요하다.
