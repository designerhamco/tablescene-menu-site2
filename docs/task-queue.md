...
- `IN_PROGRESS` 판매 가능한 멀티페이지 다이닝 `오브 테이블` — 커버 선택 노출, 최대 10개 메뉴 페이지, 1·2열/정렬 설정, 코스·직접 메뉴 혼합, 페이지 탭 드래그 정렬, 고정 dot 이동과 공개 renderer를 구현. 템플릿명과 스타터 이름을 통일하고 시그니처·단품·드링크 완성형 스타터, 전용 커버 이미지, 기본정보 매장명/로고 커버 표시를 연결. 페이지 제목은 유지하면서 메뉴명·가격·설명과 코스·항목 간격을 태블릿 우선 유동 스케일로 확대하고 1180×820·820×1180·1440×900·390×844 route QA를 완료. 커버 이미지에는 선택 배경색을 기본 75%로 겹치고 편집 화면에서 0~100%로 조절하도록 연결. additive migration과 generated types 갱신은 2026-08-30 사용자 승인 아래 Production 완료. 최종 시각 확인·pilot 지정·판매 노출은 별도 승인 필요
