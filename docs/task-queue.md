...
- `IN_PROGRESS` 고객센터 전용 디자인 — FAQ·AI 상담·1:1 문의의 1차 정보 구조와 무그림자 표면 체계는 완료. 중복된 FAQ 진입 카드와 하단 검정 CTA를 제거하고, 고객센터의 AI 상담 카드는 별도 페이지 이동 없이 공통 퀵 상담창을 직접 열도록 연결했다. 검색·모바일 밀도·문의 내역 화면의 2차 보정은 계속 진행
- `DONE` 메뉴판 종료 상태 문구 — 내부 상태값과 보관·복구 정책은 유지하면서, MY/메뉴판의 탭·카드·빈 상태와 접근 제한 안내에 노출되던 `삭제됨`을 고객 친화적인 `종료됨`으로 통일
- `IN_PROGRESS` AI 상담 콘텐츠·대화 UX 보정 — 현재 판매 정책·QR·AI 크레딧 지식과 추천 질문은 최신화. 2026-09-16 Production에서 추천 질문 4종을 실제 전송해 가격·기능 경계·QR·스마트호출·AI 크레딧·디스플레이 업로드 제한과 답변 길이를 확인했다. `docs/runbooks/ai-support-production-qa.md`. 다음은 1:1 문의 전환과 모바일 채팅창 밀도 검증
