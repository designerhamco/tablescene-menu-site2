...
- `DONE` 단일페이지 언어 UI와 대표 영역 편집 계약 정리 — 오브 커피·모카 포레스트 PC·태블릿의 언어 변경 UI를 가게명 위 독립 행으로 이동했다. 대표 영역 편집 화면은 최대 5개의 `대표 이미지` 관리와 이미지별 `대표 상품` 연결을 서로 다른 카드로 완전히 분리한다. 상품을 연결하지 않거나 상품 정보 표시를 꺼도 이미지는 유지하고 공개 메뉴판에는 이미지 단독으로 표시한다.
- `DONE` 60분 재고 마감 디지털 타이머 — 오브 커피·모카 포레스트·선데이 라인의 재고 마감 스타터를 초 단위 `MM:SS`/`HH:MM:SS` 타이머로 표시하고, 상대 너비·tabular 숫자와 현재 언어 접근성 이름으로 레이아웃 안정성과 다국어 안내를 유지한다.
