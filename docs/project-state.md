...
- 활성 단일페이지 4종은 편집기 내부용 페이지명을 디자인 문구로 렌더링하지 않는다. 카테고리명은 메뉴명보다 실제 계산 크기 기준 1.35배 이상을 유지하고, 선데이 라인은 카테고리를 더 키우지 않고 상품명과 설명의 위계를 보정했다. 메뉴 상품명과 대표 영역 상품명은 동일한 밀도·viewport 크기 공식을 사용하며, 메뉴 설명·안내 1/2/3·대표 메뉴 설명·가게 설명은 동일한 viewport 기반 글자 공식을 사용한다. 7개 판매 템플릿의 영어·중국어·일본어 21개 route와 확대·축소 재계산은 로컬 실제 브라우저 검사를 통과했다.
