...
현재 작업 브랜치: `codex/aube-b-sidebar-page-rows`
...
- 두 번째 멀티페이지 레이아웃 `오브 테이블 B`는 같은 데이터·편집·번역·스마트호출 계약을 재사용하되 PC·태블릿 왼쪽 페이지 메뉴와 모바일 상단 스와이프 탭을 사용한다. 판매·교체 후보에는 아직 노출하지 않는 `coming_soon` 상태
...
- 오브 테이블 멀티페이지 기반:
  ...
  - B타입은 A타입의 커버·페이지·코스·직접 메뉴·타이포·번역·스마트호출 데이터를 그대로 사용한다. PC·태블릿에서는 네이비 왼쪽 페이지 메뉴와 오른쪽 스크롤 콘텐츠를, 모바일에서는 공통 헤더 바로 아래 가로 스와이프 페이지명 탭을 제공하며 하단 dot·Prev·Next·페이지 swipe는 사용하지 않는다. PC·태블릿 사이드바에는 편집 데이터인 커버 제목과 페이지명만 노출하고 임의 번호·보조 문구를 추가하지 않는다. 각 항목은 사이드바 전체 폭을 채우는 상단 정렬 행 버튼으로 표시한다
  - B타입 production build를 1440×900·1180×820·390×844에서 확인했다. 각 breakpoint의 탐색 UI 전환, 페이지 클릭, 스마트호출 진입, 가로 overflow 없음과 A타입 하단 탐색 회귀가 통과했다
