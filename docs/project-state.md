...
기준선: `tablescene-next`의 `cc266df` (PR #224 병합 후)
...
- 단일페이지 4종의 스타터는 화면 밀도를 기준으로 카테고리별 메뉴 수를 보정하고 중복 할인 문구를 제거했다. 생성 직후 한국어·영어·중국어·일본어 번역과 활성 locale을 기본 포함하며, 핵심 글자·간격은 고정 px 대신 viewport·container 기반 유동값으로 반응한다. 모카 포레스트의 언어 전환 UI는 어두운 배경에서 보이도록 흰색 계열로 고정했다. `만들기`에서 여는 스타터 미리보기에도 단일·멀티페이지 6종과 Display 1종의 완성된 4개 언어 데이터를 직접 연결해 언어 버튼이 화면 전체 실제 문구를 전환한다.
- 미리보기 도움말은 박스 없는 `오늘 하루 보지 않기` 체크박스와 `닫기`를 분리해 체크 후 닫을 때만 하루 숨김을 저장한다. 브라우저 안내는 URL 입력창처럼 보이는 긴 둥근 사각형, 회색으로 채운 원형 프로필, 더보기·확대/축소 위치를 표시하며 Guest 문구는 사용하지 않는다. PC·태블릿·모바일 선택기는 기본으로 열려 있고 화살표로만 접고 펼친다. 마우스 근접 효과는 사용하지 않으며, 접힐 때 별도 짧은 pill로 바뀌지 않고 같은 최대 384px 너비·높이의 바가 위로 이동해 얇은 손잡이만 남는다. 펼침·접힘은 같은 반투명 검정 배경을 유지한다. Display는 기기 선택 안내 없이 브라우저 확대/축소 안내만 사용하고, 하단 페이지 제어는 포인터가 화면 아래 영역에 들어올 때만 나타난다.
