...
기준 커밋: `d3d132c` (`PR #52` 병합)

현재 작업 브랜치: `codex/operations-dashboard-navigation`

## 현재 작업

- 공통 헤더의 `나의 메뉴판` 옆에 전용 `매장 운영` 진입을 배치하고 메뉴판 카드의 중복 진입 버튼 제거
- 운영 첫 화면을 진행 주문·대기 호출·운영 테이블·오늘 매출과 최근 이력을 보여주는 대시보드로 구성
- 공개 중이고 이용 기간과 주문 제출·Order Dashboard runtime이 활성화된 다이닝 메뉴판만 상단 탭에 노출
- 주문·호출·테이블·매출 상세 화면에 동일한 메뉴판 탭과 왼쪽 운영 내비게이션 유지
