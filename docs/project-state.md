...
- 활성 단일페이지 4종은 편집기 내부용 페이지명을 디자인 문구로 렌더링하지 않는다. 오브 커피·모카 포레스트는 선데이 라인의 기기별 타이포그래피 위계를 공통 계약으로 사용한다. 메뉴명을 1로 둘 때 `카테고리명/보조언어명/설명/가격/옵션명` 비율은 PC `1.50/0.64/0.71/1.00/0.56`, 태블릿 `1.40/0.63/0.72/1.04/0.53`, 모바일 `1.34/0.68/0.81/1.04/0.62`이며, 각 템플릿의 안전 맞춤 엔진이 선택한 메뉴명 크기를 기준으로 함께 유동한다. 가게명도 선데이 라인과 같은 viewport 기반 계산식을 사용하고 태블릿에서는 세 템플릿 모두 0.94 보정 배율을 적용한다. 선데이 라인은 PC·태블릿의 상단 정보행과 메뉴행 사이만 유동 간격을 소폭 늘린다. 라운드 포커스 PC·태블릿은 좌·중앙·우 3열을 `1:1:1`로 사용하면서 외곽과 열 내부의 유동 여백을 넓혀 답답함을 줄였다. 상품명·텍스트칩·설명·가격은 일반 메뉴와 대표 영역이 동일한 역할 토큰과 스타일을 공유하며, 대표 영역도 PC·태블릿·모바일 모두 대응 메뉴 역할과 같은 계산 크기를 사용한다. 텍스트칩은 상품명 계산 크기의 0.62배로 모든 기기에서 직접 연동한다. 카테고리 설명·가게 설명·안내 1/2/3은 상품 설명과 같은 유동 타이포그래피 계약을 유지한다. 이 장치·역할 계약은 이후 같은 단일페이지 엔진에 추가되는 스킨에도 기본 적용한다.
- 오브 커피·모카 포레스트의 PC·태블릿 언어 변경 UI는 긴 가게명과 가로 공간을 경쟁하지 않도록 가게명 위의 독립 행에 배치한다. 오브 커피는 왼쪽, 모카 포레스트는 오른쪽 정렬을 사용한다. 대표 영역 편집기는 `대표 이미지`와 `대표 상품`을 서로 다른 카드로 분리한다. 이미지 카드에서는 최대 5개 이미지의 추가·교체·삭제·순서만 관리하고, 상품 카드에서는 각 이미지 순서별 상품 연결과 상품 정보 표시 여부만 관리한다. 대표 슬라이드의 이미지는 상품 연결 유무와 관계없이 보존하고, 상품 정보가 켜져 있을 때만 현재 노출 가능한 메뉴와 연결한다.
