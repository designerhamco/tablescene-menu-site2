...
- 미리보기 도움말은 박스 없는 `오늘 하루 보지 않기` 체크박스와 `닫기`를 분리해 체크 후 닫을 때만 하루 숨김을 저장한다. 단일·멀티페이지는 브라우저 자체 확대 대신 상단 바의 `− / 현재 배율 / +` 전용 제어를 사용한다. 75·90·100·110·125% 단계에서 기기 프레임 외곽은 유지하고 iframe의 실제 레이아웃 viewport를 역비율로 바꾼 뒤 화면 크기에 맞춰 렌더링해 텍스트·열·간격·fit 엔진이 실제 브라우저 확대처럼 다시 계산된다. 현재 배율 버튼은 100%로 초기화하고 기기별 배율은 세션 동안 따로 기억한다. `태블릿 → PC → 모바일` 선택기는 기본으로 열려 있고 화살표로만 접고 펼치며, 접힐 때 같은 최대 432px 너비의 바가 위로 이동해 얇은 손잡이만 남는다. 태블릿 방향은 한 개의 화면 회전 아이콘으로 왕복 전환한다. 바 전체는 48% 검정과 2px 최소 블러로 뒤 메뉴판을 비치게 한다. Display는 기존처럼 URL 입력창·프로필·더보기를 포함한 브라우저 확대/축소 안내만 사용하고, 하단 페이지 제어는 포인터가 화면 아래 영역에 들어올 때만 나타난다.
