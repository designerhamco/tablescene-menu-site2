...
기준선: `tablescene-next`의 `2d5ed93` (PR #218 병합 후)
...
- 단일페이지 데스크톱 메뉴판은 fit/fill 후보를 측정하는 동안 `최적의 배치를 찾고 있어요` 로딩 레이어로 중간 DOM 변화를 가린다. 글꼴 로딩·보드/메뉴 resize·스타일 후보 변경이 멈춘 뒤 실제 DOM 잘림 검사를 통과해야만 `ready`로 공개하고, 계속 잘림이 남으면 잘린 메뉴 대신 실패 안내를 표시한다. 모카 포레스트는 기존 DOM 잘림 검증과 중복되던 2차 최종 확대 보정을 생략해 로컬 1440×900 기준 안정화 시간을 약 21초에서 14초로 줄였다.
