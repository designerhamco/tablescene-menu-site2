...
- 단일페이지 위젯은 카테고리와 같은 최상위 콘텐츠 블록으로 취급한다. 분리선 없는 `카테고리→위젯`과 `위젯→위젯`은 해당 스킨의 카테고리 경계 간격을 그대로 사용하고, `위젯→카테고리`는 일반 카테고리 경계와 동일한 분리선/무분리선 규칙을 따른다. 마지막 열 하단 도킹이 남는 공간을 더 벌릴 수는 있지만 이 최소 의미 간격을 줄일 수는 없다.
...
- 모카 포레스트의 마지막 열 하단 위젯은 하단 정렬을 유지하되 `clamp(5.5rem, board-padding * 3, 7.5rem)`의 유동 푸터 안전 여백을 사용해 푸터 금지 영역을 침범하지 않는다.
