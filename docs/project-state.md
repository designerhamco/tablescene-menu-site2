...
기준선: `tablescene-next`의 `91deeb6` (PR #225 병합 후)
...
- 단일페이지 4종의 스타터는 화면 밀도를 기준으로 카테고리별 메뉴 수를 보정하고 중복 할인 문구를 제거했다. 생성 직후 한국어·영어·중국어·일본어 번역과 활성 locale을 기본 포함하며, 핵심 글자·간격은 고정 px 대신 viewport·container 기반 유동값으로 반응한다. 모카 포레스트의 언어 전환 UI는 어두운 배경에서 보이도록 흰색 계열로 고정했다. 모바일에서는 4종 모두 언어 UI를 제목 옆에서 분리해 제목 위 새 행 오른쪽에 배치한다. 태블릿 가로 구간은 PC보다 작은 맞춤 글자 상한과 역할별 카테고리·메뉴명·가격·설명 비율을 사용한다. `만들기`에서 여는 스타터 미리보기에도 단일·멀티페이지 6종과 Display 1종의 완성된 4개 언어 데이터를 직접 연결해 언어 버튼이 화면 전체 실제 문구를 전환한다.
...
- 활성 단일페이지 4종은 편집기 내부용 페이지명을 디자인 문구로 렌더링하지 않는다. 카테고리명은 메뉴명보다 실제 계산 크기 기준 1.35배 이상을 유지하고, 메뉴 설명·안내 1/2/3·대표 메뉴 설명·가게 설명은 동일한 viewport 기반 글자 공식을 사용한다. 7개 판매 템플릿의 영어·중국어·일본어 21개 route와 확대·축소 재계산은 로컬 실제 브라우저 검사를 통과했다.
