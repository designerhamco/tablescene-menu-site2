...
- 오브 커피·모카 포레스트의 PC·태블릿 언어 변경 UI는 긴 가게명과 가로 공간을 경쟁하지 않도록 가게명 위의 독립 행 오른쪽에 배치한다. 대표 영역 편집기는 `대표 이미지`와 `대표 상품`을 서로 다른 카드로 분리한다. 이미지 카드에서는 최대 5개 이미지의 추가·교체·삭제·순서만 관리하고, 상품 카드에서는 각 이미지 순서별 상품 연결과 상품 정보 표시 여부만 관리한다. 대표 슬라이드의 이미지는 상품 연결 유무와 관계없이 보존하고, 상품 정보가 켜져 있을 때만 현재 노출 가능한 메뉴와 연결한다.
- 오브 커피·모카 포레스트·선데이 라인의 60분 재고 마감 스타터는 초 단위 디지털 카운트다운을 사용한다. 1시간 이상은 `01:00:00`, 1시간 미만은 `59:59` 형식으로 표시하고 8ch 상대 너비와 tabular 숫자를 사용해 숫자가 바뀌어도 메뉴 배치가 흔들리지 않는다. 시각 타이머와 별도로 현재 언어의 남은 시간 접근성 이름을 제공하고, 종료 시 할인 칩·가격·타이머를 함께 숨긴다.
