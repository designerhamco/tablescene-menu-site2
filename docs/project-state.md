...
기준 커밋: `7e9e27c` (`PR #55` 병합)

현재 작업 브랜치: `codex/account-role-profile`

## 현재 작업

- 메뉴판 카드에 반복되던 `사장` 표시를 계정 로그인 정보 카드로 이동
- 사장 계정은 `사장`, 직원 전용 계정은 `직원`을 계정 역할로 표시하고 메뉴판별 직원 세부 역할은 기존 카드에서 유지
