...
- 단일페이지 4종의 스타터는 화면 밀도를 기준으로 카테고리별 메뉴 수를 보정하고 중복 할인 문구를 제거했다. 모카 포레스트는 디저트를 6개에서 4개로 줄여 칼럼 밀도를 맞춰다. 생성 직후 한국어·영어·중국어·일본어 번역과 활성 locale을 기본 포함하며, 핵심 글자·간격은 고정 px 대신 viewport·container 기반 유동값으로 반응한다. 모카 포레스트의 언어 전환 UI는 어두운 배경에서 보이도록 흰색 계열로 고정했다. 모바일에서는 4종 모두 언어 UI를 제목 옆에서 분리해 제목 위 새 행 오른쪽에 배치한다. 미리보기는 iframe 폭으로 breakpoint를 추측하지 않고 `pc`·`tablet`·`mobile` 기기 정보를 renderer에 직접 전달한다. 따라서 1180×820 태블릿 가로는 desktop CSS breakpoint를 넘어도 PC보다 작은 맞춤 글자와 여백을 사용한다. `만들기`에서 여는 스타터 미리보기에도 단일·멀티페이지 6종과 Display 1종의 완성된 4개 언어 데이터를 직접 연결해 언어 버튼이 화면 전체 실제 문구를 전환한다.
...
- 활성 단일페이지 4종은 편집기 내부용 페이지명을 디자인 문구로 렌더링하지 않는다. PC 카테고리명은 메뉴명보다 실제 계산 크기 기준 1.35배 이상을 유지하되 선데이 라인 태블릿은 과도하게 키우지 않고 1.2배 이상의 위계를 유지한다. 모카 포레스트 카테고리는 기존 유동 계산값에서 1.08배 보정했고, 선데이 라인은 PC·태블릿의 상단 정보행과 메뉴행 사이만 유동 간격을 소폭 늘렸다. 라운드 포커스 PC·태블릿은 좌·중앙·우 3열을 `1:1:1`로 사용한다. 상품명·텍스트칩·설명·가격은 일반 메뉴와 대표 영역이 동일한 역할 토큰과 스타일을 공유하며 대표 영역만 1.25배 크게 표시한다. 카테고리 설명·가게 설명·안내 1/2/3은 상품 설명과 같은 유동 타이포그래피 계약을 유지한다. 단일페이지 4종의 PC·태블릿·모바일 12개 역할 연동 화면과 7개 판매 템플릿의 영어·중국어·일본어 21개 route, 미리보기 툴바 1개를 합친 로컬 브라우저 34개 시나리오 및 공개 화면 31개 시나리오가 실패 0건으로 통과했다.
