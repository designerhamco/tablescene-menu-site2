...
기준선: `tablescene-next`의 `397592b` (PR #219 병합 후)
...
- 단일페이지 4종의 스타터는 화면 밀도를 기준으로 카테고리별 메뉴 수를 보정하고 중복 할인 문구를 제거했다. 생성 직후 한국어·영어·중국어·일본어 번역과 활성 locale을 기본 포함하며, 핵심 글자·간격은 고정 px 대신 viewport·container 기반 유동값으로 반응한다. 모카 포레스트의 언어 전환 UI는 어두운 배경에서 보이도록 흰색 계열로 고정했다.
- 미리보기 도움말은 박스 없는 `오늘 하루 보지 않기` 체크박스와 `닫기`를 분리해 체크 후 닫을 때만 하루 숨김을 저장한다. 브라우저 안내는 URL 입력창처럼 보이는 긴 둥근 사각형, 회색으로 채운 원형 프로필, 더보기·확대/축소 위치를 표시하며 Guest 문구는 사용하지 않는다. PC·태블릿·모바일 선택기는 기본으로 열려 있고 화살표로만 접고 펼친다. 마우스 근접 효과는 사용하지 않으며, 접힌 상태는 더 얇은 상단 중앙 pill과 펼친 상태와 같은 배경 투명도를 사용한다. Display는 기기 선택 안내 없이 브라우저 확대/축소 안내만 사용하고, 하단 페이지 제어는 포인터가 화면 아래 영역에 들어올 때만 나타난다.
- 단일페이지 데스크톱 메뉴판은 fit/fill 후보를 측정하는 동안 `최적의 배치를 찾고 있어요` 로딩 레이어로 중간 DOM 변화를 가린다. 글꼴 로딩·보드/메뉴 resize·스타일 후보 변경이 멈춘 뒤 실제 DOM 잘림 검사를 통과해야만 `ready`로 공개한다. 정상 판매 템플릿은 모두 `ready`를 통과해야 하며, 방어적 재검증 후에도 잘림이 남는 예외적 상황에서만 잘린 메뉴를 숨기고 `새로고침`으로 재계산하는 복구 안내를 표시한다. 모카 포레스트는 기존 DOM 잘림 검증과 중복되던 2차 최종 확대 보정을 생략해 로컬 1440×900 기준 안정화 시간을 약 21초에서 14초로 줄였다.
