...
기준선: `tablescene-next`의 `51b8b3a` (PR #228 병합 후)
...
- 미리보기 도움말은 박스 없는 `오늘 하루 보지 않기` 체크박스와 `닫기`를 분리해 체크 후 닫을 때만 하루 숨김을 저장한다. 브라우저 안내는 URL 입력창처럼 보이는 긴 둥근 사각형, 회색으로 채운 원형 프로필, 더보기·확대/축소 위치를 표시하며 Guest 문구는 사용하지 않는다. `태블릿 → PC → 모바일` 선택기는 기본으로 열려 있고 화살표로만 접고 펼친다. 마우스 근접 효과는 사용하지 않으며, 접힐 때 별도 짧은 pill로 바뀌지 않고 같은 최대 384px 너비의 바가 위로 이동해 얇은 손잡이만 남는다. 태블릿의 가로·세로 선택은 기기 버튼과 같은 한 행의 작은 segmented control로 표시해 메뉴판을 가리는 높이를 줄인다. 바 전체는 48% 검정과 2px 최소 블러로 뒤 메뉴판을 비치게 하고, 비활성 버튼에만 별도 반투명 받침을 사용해 글자 대비를 유지한다. Display는 기기 선택 안내 없이 브라우저 확대/축소 안내만 사용하고, 하단 페이지 제어는 포인터가 화면 아래 영역에 들어올 때만 나타난다.
- 단일페이지 데스크톱 메뉴판은 fit/fill 후보를 측정하는 최초 진입·데이터 변경 때만 `최적의 배치를 찾고 있어요` 로딩 레이어로 중간 DOM 변화를 가린다. 한 번 `ready`가 된 뒤의 브라우저 확대·축소나 화면 resize에서는 현재 메뉴판을 유지한 채 뒤에서 재계산해 로딩 섬광을 보이지 않는다. 최종 DOM이 넘치면 메뉴 글자·간격에 종단 안전 축소를 적용해 다시 검증하며, 최소 안전 배율에서도 잘림이 남는 예외적 상황에서만 메뉴를 숨기고 `새로고침`으로 재계산하는 복구 안내를 표시한다.
- 활성 단일페이지 4종은 편집기 내부용 페이지명을 디자인 문구로 렌더링하지 않는다. 카테고리명은 메뉴명보다 실제 계산 크기 기준 1.35배 이상을 유지하고, 선데이 라인은 카테고리를 더 키우지 않고 상품명과 설명의 위계를 보정했다. 상품명·텍스트칩·설명·가격은 각각 일반 메뉴와 대표 영역이 동일한 역할 토큰과 계산 스타일을 사용한다. 상품 설명·대표 설명·카테고리 설명·가게 설명·안내 1/2/3도 하나의 유동 타이포그래피 계약으로 연결하며, 가격은 서로 다른 container에서도 같은 크기가 되도록 viewport 기준 유동값을 사용한다. 단일페이지 4종의 PC·태블릿·모바일 12개 역할 연동 화면과 7개 판매 템플릿의 영어·중국어·일본어 21개 route, 미리보기 툴바 1개를 합친 로컬 브라우저 34개 시나리오 및 공개 화면 31개 시나리오가 실패 0건으로 통과했다.
