...
- 활성 단일페이지 4종은 편집기 내부용 페이지명을 디자인 문구로 렌더링하지 않는다. PC 카테고리명은 메뉴명보다 실제 계산 크기 기준 1.35배 이상을 유지하되 선데이 라인 태블릿은 과도하게 키우지 않고 1.2배 이상의 위계를 유지한다. 오브 커피는 모든 기기에서 카테고리명이 메뉴명보다 최소 1.24배 크게 보이도록 안전 하한을 두고, 모카 포레스트 카테고리는 기존 유동 계산값에서 1.14배 보정한다. 두 템플릿의 보조언어명·설명·옵션명은 메뉴명 대비 각각 최소 0.65·0.72·0.58 비율을 유지한다. 선데이 라인은 PC·태블릿의 상단 정보행과 메뉴행 사이만 유동 간격을 소폭 늘렸다. 라운드 포커스 PC·태블릿은 좌·중앙·우 3열을 `1:1:1`로 사용하면서 외곽과 열 내부의 유동 여백을 넓혀 답답함을 줄였다. 상품명·텍스트칩·설명·가격은 일반 메뉴와 대표 영역이 동일한 역할 토큰과 스타일을 공유한다. 대표 영역의 계산 크기만 PC·태블릿에서 1.25배이고 모바일에서는 일반 메뉴 역할과 같은 크기다. 카테고리 설명·가게 설명·안내 1/2/3은 상품 설명과 같은 유동 타이포그래피 계약을 유지한다. 이 장치·역할 계약은 이후 같은 단일페이지 엔진에 추가되는 스킨에도 기본 적용한다.
