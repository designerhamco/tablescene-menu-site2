...
기준선: `tablescene-next`의 `e79679f` (PR #230 병합 후)
...
- 오브 커피·모카 포레스트의 PC·태블릿 언어 변경 UI는 긴 가게명과 가로 공간을 경쟁하지 않도록 가게명 위의 독립 행 오른쪽에 배치한다. 대표 영역은 `대표 영역 사용`과 `대표 상품 정보 표시`를 분리해, 이미지만 등록하고 상품명·설명·칩·가격은 숨기는 이미지 단독 구성을 지원한다. 대표 슬라이드의 이미지는 상품 연결 유무와 관계없이 보존하고, 상품 정보가 켜져 있을 때만 현재 노출 가능한 메뉴와 연결한다.
