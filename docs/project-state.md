...
현재 작업 브랜치: `codex/aube-tablet-typography`
...
- 오브 테이블 멀티페이지 기반:
  ...
  - 모든 스타터 페이지는 중앙 헤더·왼쪽 메뉴 정보 규칙을 공유하며 2열 코스 묶음, 확대한 타이포·여백·가격 리더선으로 실제 파인다이닝 메뉴판 밀도를 반영
  - 페이지 제목 비율은 유지하고 메뉴명 22~30px·가격 18~24px·설명 16~20px을 태블릿 우선 유동 스케일로 확대. 코스·항목 간격도 화면 폭에 따라 조절하며 1180×820, 820×1180, 1440×900, 390×844에서 가로 overflow 없이 QA 완료
