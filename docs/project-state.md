...
- 미리보기 도움말은 박스 없는 `오늘 하루 보지 않기` 체크박스와 `닫기`를 분리해 체크 후 닫을 때만 하루 숨김을 저장한다. 브라우저 안내는 URL 입력창처럼 보이는 긴 둥근 사각형, 회색으로 채운 원형 프로필, 더보기·확대/축소 위치를 표시하며 Guest 문구는 사용하지 않는다. PC·태블릿·모바일 선택기는 기본으로 열려 있고 화살표로만 접고 펼친다. 마우스 근접 효과는 사용하지 않으며, 접힐 때 별도 짧은 pill로 바뀌지 않고 같은 최대 384px 너비·높이의 바가 위로 이동해 얇은 손잡이만 남는다. 펼침·접힘은 같은 반투명 검정 배경을 유지한다. 태블릿의 가로·세로 선택은 기기 행 아래 별도 중앙 정렬 segmented 행으로 표시한다. Display는 기기 선택 안내 없이 브라우저 확대/축소 안내만 사용하고, 하단 페이지 제어는 포인터가 화면 아래 영역에 들어올 때만 나타난다.
