...
기준선: `tablescene-next`의 `27c20af` (PR #231 병합 후)

## 현재 상태와 다음 선행 조건

- 단일페이지 판매 템플릿의 현재 표시명은 `REAL MATCHA`, `모카 포레스트`, `선데이 로스터스`, `라운드 포커스`다. 기존 저장 데이터와 교체·구매 계약의 내부 키 `cafe_design_a`, `cafe_sunday_line_a`는 유지하고, 공개 스타터 미리보기에는 `/templates/cafe_real_matcha_a/preview`, `/templates/cafe_sunday_roasters_a/preview` 별칭을 사용한다. 옛 내부 키 route도 기존 링크 호환을 위해 계속 해석한다.
- 리얼 맛차 스타터는 맛차 중심 시그니처와 단일 가격, `#2AA36D` 칩, 카테고리명 아래 선을 사용하며 상품 썸네일·NEW 칩·카테고리 경계선·원두 선택 설명을 사용하지 않는다. 모카 포레스트 스타터는 `#EFE4D0` 베이지, 상품 연결 없는 대표 이미지 1개, 3:4 이미지 위젯, 단일 가격을 사용하고 기간 할인 외 칩·보조언어명·상품설명은 의도적으로 표시하지 않는다. 선데이 로스터스는 기존 디자인·가게명과 데이터를 유지하며 표시명과 공개 미리보기 링크만 통일했다.
- 단일·멀티페이지 미리보기는 최초 진입 시 태블릿 가로를 기본으로 열고 선택기를 `태블릿 → PC → 모바일` 순서로 표시한다. URL에 유효한 기기가 명시되면 해당 값을 우선하며 Display는 PC 전용 기본값을 유지한다.
...
- 공개 회귀 QA는 홈·신청·고객지원·인증·약관과 판매 템플릿을 PC `1440×900`·모바일 `390×844`로 순회한다. HTTP·빈 화면·가로 overflow·깨진 이미지·console/page error·같은 출처 요청 실패와 axe-core WCAG A/AA `critical`·`serious` 위반을 차단한다. 2026-09-16 PR #200 배포 후 Production 31개 화면과 PR #216 병합 후 로컬 31개 화면에서 각각 실패 0건을 확인했다. 로그인 이후 화면은 전용 QA 계정만 사용하는 별도 읽기 전용 runner로 마이페이지·편집기·매장 운영·직원 관리의 세션 유지, 잘린 액션과 동일 오류 범위를 검사하며 service-role key나 고객 계정을 사용하지 않는다.
- 단일페이지 4종의 스타터는 화면 밀도를 기준으로 카테고리별 메뉴 수를 보정하고 중복 할인 문구를 제거했다. 모카 포레스트는 디저트를 6개에서 4개로 줄여 칼럼 밀도를 맞췄다. 생성 직후 한국어·영어·중국어·일본어 번역과 활성 locale을 기본 포함하며, 핵심 글자·간격은 고정 px 대신 viewport·container 기반 유동값으로 반응한다. 네 템플릿의 모든 스타터 메뉴는 주 언어명과 보조언어명을 함께 가지며 보조언어명은 공통 보조서체의 600 굵기를 사용한다. 모카 포레스트의 언어 전환 UI는 어두운 배경에서 보이도록 흰색 계열로 고정했다. 모바일에서는 4종 모두 언어 UI를 제목 옆에서 분리해 제목 위 새 행 오른쪽에 배치한다. 미리보기는 iframe 폭으로 breakpoint를 추측하지 않고 `pc`·`tablet`·`mobile` 기기 정보를 renderer에 직접 전달하며 태블릿은 같은 역할 토큰에 1.12 장치 배율을 추가한다. `만들기`에서 여는 스타터 미리보기에도 단일·멀티페이지 6종과 Display 1종의 완성된 4개 언어 데이터를 직접 연결해 언어 버튼이 화면 전체 실제 문구를 전환한다.
- 메뉴판 간격 계약은 단일페이지·Display의 `canvas-fit`과 멀티페이지의 `editorial-scroll`로 분리한다. 전자는 화면·컨테이너와 안전 맞춤 결과에 따라 유동적으로 채우고, 후자는 최대 읽기 폭과 자연스러운 스크롤을 유지해 빈 공간을 억지로 채우지 않는다. 단일페이지는 메뉴 간격을 `1`로 두고 카테고리명과 첫 메뉴 간격도 `1`, 분리선이 없는 카테고리 경계는 시각적으로 분명한 `2.2`를 사용한다. 화면 맞춤은 기준 메뉴 간격에만 한 번 적용하며 의미 비율에는 다시 곱하지 않는다. Display도 카테고리명 아래 간격과 메뉴 간격을 같은 행 예산으로 계산한다. 멀티페이지의 페이지·열·섹션·아이템 여백은 별도 유동 토큰을 사용하되 콘텐츠 개수 기반 fit/fill 축소·확대는 하지 않는다. 새 템플릿은 두 계약 중 하나를 루트에 선언해야 한다. `docs/template-spacing-contract.md`
- 단일페이지 위젯은 카테고리와 같은 최상위 콘텐츠 블록으로 취급한다. 분리선 없는 `카테고리→위젯`과 `위젯→위젯`은 해당 스킨의 카테고리 경계 간격을 그대로 사용하고, `위젯→카테고리`는 일반 카테고리 경계와 동일한 분리선/무분리선 규칙을 따른다. 위젯별 `settings.placement`에서 `flow`(콘텐츠 이어붙이기)와 `bottom`(마지막 열 하단 정렬)을 저장하며 기존 값이 없는 위젯과 새 위젯은 현재 동작을 보존하는 `bottom`을 기본으로 한다. 하단 위젯은 PC·태블릿의 마지막 열, 모바일의 콘텐츠 끝으로 모이고 안내사항이 있으면 그 바로 위에 쌓인다. `위젯↔안내사항` 간격은 해당 스킨의 `카테고리↔카테고리` 간격과 동일하다.
- 라운드 포커스는 큰 카테고리명과 제목형 메뉴 행의 시각 보정을 위해 `카테고리명→첫 메뉴 1.15 / 카테고리 경계 2.8`을 사용한다. 모카 포레스트는 공통 `1 / 2.2`를 사용하며 메뉴별 추가 하단 padding을 더하지 않는다.
- 모카 포레스트를 포함한 단일페이지의 하단 위젯과 안내사항은 마지막 열의 같은 흐름 안에서 배치한다. 위젯 위의 남는 공간으로 하단 정렬하고, 위젯 아래에는 별도 큰 푸터 예약 여백 대신 해당 스킨의 카테고리 경계 간격만 둔다.
- 모카 포레스트는 열의 반응형 하단 padding 자체를 메뉴 영역 안전 여백으로 인정한다. 해당 padding 바깥에 중복 안전 간격을 요구하지 않지만 실제 사각형·스크롤·위젯·푸터·오른쪽 끝 넘침 검사는 그대로 유지한다.
- 모카 포레스트에 위젯이 있고 데스크톱 fit 영역이 `760px` 이상이면 메뉴 열은 3열만 사용한다. 중국어·일본어 긴 문구가 시뮬레이션상 2열을 통과한 뒤 실제 DOM 검사에서 실패하는 잘못된 구조 변경을 허용하지 않는다.
- `orderedBalancedFit`은 각 pass에서 전체 후보를 비교한 뒤 서로 다른 안전 후보를 3개 이상 순회하면 현재의 비오버플로·비거절 후보로 수렴한다. 이후 실제 DOM 잘림 검사는 그대로 유지되어 잘린 후보는 제외하고 다시 맞춘다.
- 이미 로드 완료된 `document.fonts.ready`는 fit effect 재실행 때 후보 이력을 초기화하지 않는다. 관찰 시작 시 폰트가 실제 로딩 중이었던 경우에만 완료 후 이력을 한 번 초기화하고 재측정한다.
- 미리보기 최초 도움말은 URL 입력창·프로필·더보기 메뉴가 있는 기존 브라우저 화면과 브라우저 확대·축소 안내 문구를 그대로 유지한다. 박스 없는 `오늘 하루 보지 않기` 체크박스와 `닫기`를 분리해 체크 후 닫을 때만 하루 숨김을 저장한다. 도움말을 닫은 뒤 단일·멀티페이지 상단 바에서는 브라우저 자체 확대 대신 `− / 현재 배율 / +` 전용 제어를 사용한다. 75·90·100·110·125% 단계에서 기기 프레임 외곽은 유지하고 iframe의 실제 레이아웃 viewport를 역비율로 바꾼 뒤 화면 크기에 맞춰 렌더링해 텍스트·열·간격·fit 엔진이 실제 브라우저 확대처럼 다시 계산된다. 현재 배율 버튼은 100%로 초기화하고 기기별 배율은 세션 동안 따로 기억한다. `태블릿 → PC → 모바일` 선택기는 기본으로 열려 있고 화살표로만 접고 펼치며, 접힐 때 같은 최대 432px 너비의 바가 위로 이동해 얇은 손잡이만 남는다. 태블릿 방향은 한 개의 화면 회전 아이콘으로 왕복 전환한다. 바 전체는 48% 검정과 2px 최소 블러로 뒤 메뉴판을 비치게 한다. Display는 기기 선택기 없이 같은 브라우저 확대·축소 도움말만 사용하고, 하단 페이지 제어는 포인터가 화면 아래 영역에 들어올 때만 나타난다.
- 단일페이지 데스크톱 메뉴판은 fit/fill 후보를 측정하는 최초 진입·데이터 변경 때만 `최적의 배치를 찾고 있어요` 로딩 레이어로 중간 DOM 변화를 가린다. 한 번 `ready`가 된 뒤의 브라우저 확대·축소나 화면 resize에서는 현재 메뉴판을 유지한 채 뒤에서 재계산해 로딩 섬광을 보이지 않는다. 웹폰트 준비는 1.5초 상한 뒤 안전 검증을 계속하고, ordered-balanced 엔진은 이미 확인한 안전 후보로 되돌아가는 순환을 차단한다. 모카 포레스트는 별도 패널 균형과 경쟁하던 2차 확대를 생략한다. 최종 DOM이 넘치면 메뉴 글자·간격에 종단 안전 축소를 적용해 다시 검증하며, 최소 안전 배율에서도 잘림이 남는 예외적 상황에서만 메뉴를 숨기고 `새로고침`으로 재계산하는 복구 안내를 표시한다.
- 활성 단일페이지 4종은 편집기 내부용 페이지명을 디자인 문구로 렌더링하지 않는다. 리얼 맛차는 선데이 로스터스의 기기별 타이포그래피 위계를 공통 계약으로 사용한다. 메뉴명을 1로 둘 때 `카테고리명/보조언어명/설명/가격/옵션명` 비율은 PC `1.50/0.64/0.71/1.00/0.56`, 태블릿 `1.40/0.63/0.72/1.04/0.53`, 모바일 `1.34/0.68/0.81/1.04/0.62`이며, 각 템플릿의 안전 맞춤 엔진이 선택한 메뉴명 크기를 기준으로 함께 유동한다. 모카 포레스트는 같은 유동 역할 토큰을 사용하되 현재 스타터에서 보조언어명·상품설명·옵션명을 의도적으로 생략하고 가게명만 전용 확대 비율과 설명 간격을 사용한다. 선데이 로스터스는 PC·태블릿의 상단 정보행과 메뉴행 사이만 유동 간격을 소폭 늘린다. 라운드 포커스 PC·태블릿은 좌·중앙·우 3열을 `1:1:1`로 사용하면서 외곽과 열 내부의 유동 여백을 넓혀 답답함을 줄였다. 상품명·텍스트칩·설명·가격은 해당 역할이 존재할 때 일반 메뉴와 대표 영역이 동일한 역할 토큰과 스타일을 공유한다. 텍스트칩은 상품명 계산 크기의 0.62배로 모든 기기에서 직접 연동한다. 카테고리 설명·가게 설명·안내 1/2/3은 상품 설명과 같은 유동 타이포그래피 계약을 유지한다. 이 장치·역할 계약은 이후 같은 단일페이지 엔진에 추가되는 스킨에도 기본 적용한다.
- 리얼 맛차·모카 포레스트의 PC·태블릿 언어 변경 UI는 긴 가게명과 가로 공간을 경쟁하지 않도록 가게명 위의 독립 행에 배치한다. 리얼 맛차는 가게 설명 아래 왼쪽, 모카 포레스트는 오른쪽 정렬을 사용한다. 대표 영역 편집기는 `대표 이미지`와 `대표 상품`을 서로 다른 카드로 분리한다. 이미지 카드에서는 최대 5개 이미지의 추가·교체·삭제·순서만 관리하고, 상품 카드에서는 각 이미지 순서별 상품 연결과 상품 정보 표시 여부만 관리한다. 대표 슬라이드의 이미지는 상품 연결 유무와 관계없이 보존하고, 상품 정보가 켜져 있을 때만 현재 노출 가능한 메뉴와 연결한다.
- 활성 단일페이지 4종은 모두 60분 재고 마감 스타터 할인을 포함하고 초 단위 디지털 카운트다운을 사용한다. 남은 시간이 1시간 미만이어도 항상 `00:59:59`와 같은 `HH:MM:SS` 형식으로 표시하고 8ch 상대 너비와 tabular 숫자를 사용해 숫자가 바뀌어도 메뉴 배치가 흔들리지 않는다. 시각 타이머와 별도로 현재 언어의 남은 시간 접근성 이름을 제공하고, 종료 시 할인 칩·가격·타이머를 함께 숨긴다. 라운드 포커스는 HOUSE SPECIALS 3개 모두 상품 이미지를 사용하고 ESPRESSO·MILK & CREAM에 메뉴를 1개씩 보강했으며, 선데이 라인의 일반 메뉴 목록은 상품 이미지를 사용하지 않는다. 모카 포레스트 모닝딜은 흰 배경에서도 읽히는 진한 브라운 칩·안내색을 사용한다.
- 타임세일은 활성 지원 템플릿 모두 메뉴판당 최대 50개로 편집기·capability·서버 저장 검증을 통일한다. 숫자 정상가, 할인가는 정상가 미만, 유효 기간·요일·시간, 같은 메뉴의 일정 비중복 조건은 유지하고 옵션 가격·직접 가격 문구에는 적용하지 않는다. 폰트 편집기는 실제 템플릿 기본 한글·영문 폰트를 명시하며, 사이트의 수정 불가 input·textarea는 공통 밝은 회색 읽기 전용 스타일을 사용한다. 공개 메뉴판 주소처럼 복사가 필요한 값은 같은 비활성 시각 상태에서 텍스트 선택을 허용한다.
- 다이닝 신청의 `OPEN` 코드는 하나의 공개 코드로 상품별 할인가를 결정한다. 코드를 지우면 단일 월 8,900원·연 106,800원, 멀티 월 12,900원·연 154,800원으로 복원된다. 코드를 적용하면 5,900원·63,700원·9,900원·106,900원이며, UI·preflight·청구·DB 저장·정기 갱신이 같은 확정 금액을 사용한다.
- 썸머 블루 전체 메뉴의 HOT·ICE 가격 열은 유동 너비와 최소 16px CSS 간격을 사용한다. 공개 회귀 QA는 렌더된 옵션 헤더·가격 셀 사이의 실제 간격이 12px 미만이면 실패한다.
- 썸머 블루 기본 할인은 동일 문구를 반복하지 않으면서 할인 기능임을 명확히 표시한다. 바질 크림 라떼는 `시즌할인`, 카푸치노는 `커피할인` 칩과 서로 다른 특별가 문구를 사용한다. 클래식 버터 스콘은 할인 대상에서 제외해 정상가만 표시한다.
...
- 신규 schema는 2026-08-30 사용자 승인 아래 Production에 1회 적용하고 generated types를 갱신했다. 오브 테이블·메종 마레의 판매/교체 노출과 별개로 스마트호출 실제 write는 runtime/site allowlist로 fail closed
...
- 메뉴판 미리보기 기기 프레임:
  ...
  - 도움말의 하루 숨김은 체크박스 선택 후 `닫기`를 누를 때만 저장하고, 체크하지 않은 닫기와 `Escape`는 다음 새 세션에서 다시 표시
  - 브라우저 안내는 원형 프로필·더보기·확대/축소만 표시하고, 접힌 기기 선택기는 상단 중앙 dark pill을 사용
  - Display 도움말은 브라우저 확대/축소만 안내하며 하단 페이지 제어는 화면 아래 포인터 접근 시에만 노출
...
- 활성 템플릿 저장·locale QA:
  ...
  - 단일페이지 4종은 균형 잡힌 스타터 메뉴 밀도와 중복 없는 할인 문구를 계약 테스트로 고정
  - 단일페이지 4종의 생성 스타터에 한국어·영어·중국어·일본어 번역과 활성 locale을 기본 포함
  - 단일페이지 4종의 핵심 타이포·여백을 fluid 단위로 전환하고 모카 포레스트의 언어 control을 흰색 계열로 보정
- 템플릿 교체 등급 경계:
...
- `1249d5c` — PR #216 병합: 클래식 버터 스콘을 할인 대상에서 제거하고 정상가만 표시, 썸머 블루 최종 승인 조건 충족
- `4185f2d` — PR #215 병합: 썸머 블루 할인 표시 문구 분리
- `88e158b` — PR #214 병합: 썸머 블루 할인 문구 중복 1차 정리
- `df79955` — PR #213 병합: 썸머 블루 HOT·ICE 가격 열 간격 보정
- `650b48f` — PR #212 병합: PR #207–#211 디자인 작업·최종 자동 QA와 남은 육안 판단 문서화
- `ab49175` — PR #211 병합: 미리보기 도움말·브라우저 안내·접힌 기기 선택기와 Display 하단 제어 UX 개편
- `a7c76d4` — PR #210 병합: 단일페이지 4종 유동 타이포·간격과 모카 포레스트 언어 UI 보정
- `8771486` — PR #209 병합: 단일페이지 4종 스타터의 한국어·영어·중국어·일본어 기본 번역
- `fa80792` — PR #208 병합: 단일페이지 4종 스타터 메뉴 밀도·할인 문구 최종 정리
- `6d98f7e` — PR #207 병합: 첫 미리보기 도움말 동작 자동 회귀 검증
