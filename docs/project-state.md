...
기준선: `tablescene-next`의 `1ba0e6f` (PR #220 병합 후)
...
- 미리보기 도움말은 박스 없는 `오늘 하루 보지 않기` 체크박스와 `닫기`를 분리해 체크 후 닫을 때만 하루 숨김을 저장한다. 브라우저 안내는 URL 입력창처럼 보이는 긴 둥근 사각형, 회색으로 채운 원형 프로필, 더보기·확대/축소 위치를 표시하며 Guest 문구는 사용하지 않는다. PC·태블릿·모바일 선택기는 기본으로 열려 있고 화살표로만 접고 펼친다. 마우스 근접 효과는 사용하지 않으며, 접힐 때 별도 짧은 pill로 바뀌지 않고 같은 너비·높이의 바가 위로 이동해 얇은 손잡이만 남는다. 펼침·접힘은 같은 검정 배경 투명도를 유지한다. Display는 기기 선택 안내 없이 브라우저 확대/축소 안내만 사용하고, 하단 페이지 제어는 포인터가 화면 아래 영역에 들어올 때만 나타난다.
- 단일페이지 데스크톱 메뉴판은 fit/fill 후보를 측정하는 동안 `최적의 배치를 찾고 있어요` 로딩 레이어로 중간 DOM 변화를 가린다. 글꼴 로딩·보드/메뉴 resize·스타일 후보 변경이 멈춘 뒤 실제 DOM 잘림 검사를 통과해야만 `ready`로 공개한다. 최종 DOM이 넘치면 곧바로 실패 안내로 전환하지 않고 메뉴 글자·간격에 종단 안전 축소를 적용해 다시 검증하며, 최소 안전 배율에서도 잘림이 남는 예외적 상황에서만 메뉴를 숨기고 `새로고침`으로 재계산하는 복구 안내를 표시한다. 모카 포레스트 영문 스타터를 포함한 모든 활성 카페 미리보기는 로컬 1440×900에서 `ready`와 실제 DOM 경계 검사를 통과했다.
- 선데이 라인은 화면 채움 배율이 바뀌어도 카테고리명·메뉴명/가격·설명의 위계를 함께 유지한다. PC 실제 렌더링 기준 카테고리명 22.38px, 메뉴명 15.27px, 설명 11.26px이며, 상단 좌우 문구는 11.24px로 메뉴 설명 크기를 따른다.
