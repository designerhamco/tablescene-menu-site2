...
기준 커밋: `bda481e` (`PR #56` 병합)

현재 작업 브랜치: `codex/permission-disabled-navigation`

## 현재 작업

- 직원 권한으로 이용할 수 없는 마이페이지·편집기·매장 운영 메뉴를 숨기지 않고 `권한 없음` 비활성 상태로 표시
- 직원 메뉴판 카드의 메뉴 편집·공개 메뉴판 동선을 권한 또는 공개 상태에 따라 같은 위치에서 비활성 처리
- 권한 없는 상세 URL 직접 접근은 서버 권한 검사를 유지한 채 마이페이지·매장 운영 안내로 안전하게 복귀
