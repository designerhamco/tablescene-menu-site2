...
기준 커밋: `095b35d` (`PR #54` 병합)

현재 작업 브랜치: `codex/operations-account-mobile-support`

## 현재 작업

- 매장 운영 왼쪽 영역에 AI 크레딧을 제외한 로그인 계정 정보와 로그아웃 동선 제공
- 모바일 알림을 햄버거 밖의 아이콘으로 이동하고 기존 읽지 않은 알림 수 배지 유지
- 모바일 메뉴 하단의 카카오톡 상담을 제거하고 로그인 복귀가 보장되는 `1:1 문의`와 임시 `채팅상담` 동선 제공
