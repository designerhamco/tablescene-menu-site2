...
- 미리보기 최초 도움말은 URL 입력창·프로필·더보기 메뉴가 있는 기존 브라우저 화면과 브라우저 확대·축소 안내 문구를 그대로 유지한다. `실제 배치는 기기에 따라 달라질 수 있으니, 사용할 기기에서 최종 확인해 주세요.` 안내를 함께 표시하고, 박스 없는 `오늘 하루 보지 않기` 체크박스와 `닫기`를 분리해 체크 후 닫을 때만 하루 숨김을 저장한다. 단일·멀티페이지 상단 바의 별도 배율 제어는 사용하지 않으며 `새 창에서 메뉴판 보기`가 현재 선택한 기기·태블릿 방향을 유지한 실제 renderer를 프레임 없이 연다. 모든 기기의 확대·축소는 새 창의 브라우저 기본 기능으로 통일한다. `태블릿 → PC → 모바일` 선택기는 기본으로 열려 있고 화살표로만 접고 펼치며, 접힐 때 같은 최대 432px 너비의 바가 위로 이동해 얇은 손잡이만 남는다. 태블릿 방향은 한 개의 화면 회전 아이콘으로 왕복 전환한다. 바 전체는 48% 검정과 2px 최소 블러로 뒤 메뉴판을 비치게 한다. Display는 기기 선택기 없이 같은 브라우저 확대·축소 도움말만 사용하고, 하단 페이지 제어는 포인터가 화면 아래 영역에 들어올 때만 나타난다.
