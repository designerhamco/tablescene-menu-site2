...
- 오브 테이블 멀티페이지 기반:
  ...
  - 템플릿명·스타터 매장명을 `오브 테이블`로 통일하고 시그니처·단품·드링크 3페이지의 완성형 스타터, 전용 파인다이닝 커버 이미지, 기본정보 매장명/로고 커버 표시를 연결
  - 모든 스타터 페이지는 중앙 헤더·왼쪽 메뉴 정보 규칙을 공유하며 2열 코스 묶음, 유동 타이포·여백·가격 리더선으로 실제 파인다이닝 메뉴판 밀도를 반영
  - 한글은 Pretendard, 영문은 Tenor Sans를 기본으로 고정하고 스타터 영문은 자연스러운 문장형 대소문자를 사용
  - PC 기준 페이지 제목 최대 68px·코스명 24~30px·메뉴명 17~20px·가격 15~18px·설명 13~15px로 조정. 코스·항목 간격도 화면 폭에 따라 축소·확대하며 1180×820, 820×1180, 1440×900, 390×844에서 가로 overflow 없이 QA 완료
  - 하단 탐색은 배경 없는 회색/금색 dot으로 정리하고 마우스를 사용하는 1280px 이상 PC에만 `Prev`·`Next` 버튼을 제공
