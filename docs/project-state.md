...
- 메뉴판 간격 계약은 단일페이지·Display의 `canvas-fit`과 멀티페이지의 `editorial-scroll`로 분리한다. 전자는 화면·컨테이너와 안전 맞춤 결과에 따라 유동적으로 채우고, 후자는 최대 읽기 폭과 자연스러운 스크롤을 유지해 빈 공간을 억지로 채우지 않는다. 단일페이지는 메뉴 간격을 `1`로 두고 카테고리명과 첫 메뉴 간격은 `1.05`, 분리선이 없는 카테고리 경계는 `1.6`을 사용한다. 화면 맞춤은 기준 메뉴 간격에만 한 번 적용하며 의미 비율에는 다시 곱하지 않는다. Display도 카테고리명 아래 간격과 메뉴 간격을 같은 행 예산으로 계산한다. 멀티페이지의 페이지·열·섹션·아이템 여백은 별도 유동 토큰을 사용하되 콘텐츠 개수 기반 fit/fill 축소·확대는 하지 않는다. 새 템플릿은 두 계약 중 하나를 루트에 선언해야 한다. `docs/template-spacing-contract.md`
