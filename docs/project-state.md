...
현재 작업 브랜치: `codex/dining-multipage-b-navigation`
...
- 오브 테이블 멀티페이지 기반:
  ...
  - B타입은 A타입의 커버·페이지·코스·직접 메뉴·타이포·번역·스마트호출 데이터를 그대로 사용한다. PC·태블릿에서는 네이비 왼쪽 페이지 메뉴와 오른쪽 스크롤 콘텐츠를, 모바일에서는 공통 헤더 바로 아래 가로 스와이프 페이지명 탭을 제공하며 하단 dot·Prev·Next·페이지 swipe는 사용하지 않는다
