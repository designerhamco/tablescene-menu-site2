...
기준선: `tablescene-next`의 `1249d5c` (PR #216 병합 후)
...
- 미리보기 도움말은 `오늘 하루 보지 않기` 체크박스와 `닫기`를 분리해 체크 후 닫을 때만 하루 숨김을 저장한다. 브라우저 안내는 원형 프로필과 더보기·확대/축소 위치만 표시하며 Guest 및 주소형 장식은 제거했다. 접힌 기기 선택기는 상단 중앙의 어두운 가로 pill로 표시한다. Display는 기기 선택 안내 없이 브라우저 확대/축소 안내만 사용하고, 하단 페이지 제어는 포인터가 화면 아래 영역에 들어올 때만 나타난다.
