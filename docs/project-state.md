...
현재 작업 브랜치: `codex/aube-final-motion-design-linkage`
...
- 오브 테이블 멀티페이지 기반:
  ...
  - 저장된 글자 역할 설정을 공개 renderer와 공통 호출 layer까지 전달. 오브 테이블의 `카테고리명 글자`는 페이지명과 코스명을 함께 변경하며 한글 Pretendard·영문 Tenor Sans 기본값과 역할별 webfont/color를 동일하게 사용
