...
현재 작업 브랜치: `codex/auth-email-template-runbook`
...
- 회원가입 인증·비밀번호 재설정 Supabase Auth 메일의 아티메뉴 제목·HTML과 적용·QA·롤백 절차를 저장소에 준비했다. Production Auth/SMTP 설정과 실제 수신 QA는 사람 확인 전까지 수행하지 않는다.
