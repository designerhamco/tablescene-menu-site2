...
- 오브 테이블 멀티페이지 기반:
  ...
  - 템플릿명·스타터 매장명을 `오브 테이블`로 통일하고 기본 커버 카피는 `THE MENU`·한글 설명으로 구성. 커버는 기본정보 로고가 있으면 원본 비율을 유지해 우선 표시하고 없으면 매장명을 표시한다. 시그니처·단품·드링크 3페이지의 완성형 스타터와 전용 파인다이닝 커버 이미지를 연결
  ...
  - PC 기준 페이지 제목 최대 68px·코스명 26~32px·메뉴명 17~20px·가격 15~18px·설명 13~15px로 조정. 최대 1440px 본문 폭과 유동 상하좌우 여백을 사용하며 코스·항목 간격도 화면 폭에 따라 축소·확대한다
