...
- 미리보기 도움말은 박스 없는 `오늘 하루 보지 않기` 체크박스와 `닫기`를 분리해 체크 후 닫을 때만 하루 숨김을 저장한다. 브라우저 안내는 URL 입력창처럼 보이는 긴 둥근 사각형, 회색 원 안에 밝은 머리·어깨 실루엣을 채운 프로필, 더보기·확대/축소 위치를 표시하며 Guest 문구는 사용하지 않는다. `태블릿 → PC → 모바일` 선택기는 기본으로 열려 있고 화살표로만 접고 펼친다. 마우스 근접 효과는 사용하지 않으며, 접힐 때 별도 짧은 pill로 바뀌지 않고 같은 최대 384px 너비의 바가 위로 이동해 얇은 손잡이만 남는다. 태블릿 방향은 별도 가로·세로 버튼 대신 한 개의 화면 회전 아이콘으로 왕복 전환한다. 바 전체는 48% 검정과 2px 최소 블러로 뒤 메뉴판을 비치게 하고, 비활성 버튼에만 별도 반투명 받침을 사용해 글자 대비를 유지한다. Display는 기기 선택 안내 없이 브라우저 확대/축소 안내만 사용하고, 하단 페이지 제어는 포인터가 화면 아래 영역에 들어올 때만 나타난다.
