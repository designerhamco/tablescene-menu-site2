...
기준 커밋: `2355209` (`PR #51` 병합)

현재 작업 브랜치: `codex/operations-dashboard`

## 현재 작업

- `내 메뉴판`과 같은 계정·사이드바 레이아웃을 사용하는 전용 `/mypage/operations` 매장 운영 허브
- 다이닝 메뉴판별 주문·호출·테이블·매출 진입을 한곳에 모으고 기존 상세 관리 화면은 재사용
- Display 메뉴판 제외, Owner·Manager·Order staff 등 기존 permission matrix와 runtime gate 유지
- 신규 DB migration, Production 데이터·환경변수·결제 변경 없음

## 완료된 주요 기능

- 매장 운영 정보 구조:
  - `내 메뉴판` 카드의 주문·호출·테이블·매출 개별 버튼을 `매장 운영` 진입으로 정리
  - 공통 마이페이지 계정 카드와 사이드바를 재사용하고 `매장 운영` 메뉴를 추가
  - 운영 허브에서 다이닝 메뉴판을 선택한 뒤 권한과 상품 활성화 상태에 맞는 관리 화면만 노출
  - 기존 상세 화면의 돌아가기 동선을 선택한 매장의 운영 허브로 연결
- 공개 서비스 소개 화면:
