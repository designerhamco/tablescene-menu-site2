...
- A widget is a category-level content block. Category-to-widget and widget-to-widget boundaries without a divider use the same category separation token; widget-to-category follows the template's ordinary divider/no-divider category boundary. Each widget persists `settings.placement` as `flow` or `bottom`. Bottom widgets are collected in the final column on PC/tablet and at the content end on mobile; flexible empty space may be added above the bottom stack but may not shrink a semantic boundary.
...
- Footer notices share the final column flow with bottom widgets. The stack order is category content, bottom widget group, then footer notices, and the widget-to-footer gap is exactly the skin's category separation token rather than a separate large footer reserve.
