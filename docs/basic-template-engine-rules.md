...
- A widget is a category-level content block. Category-to-widget and widget-to-widget boundaries without a divider use the same category separation token; widget-to-category follows the template's ordinary divider/no-divider category boundary. Bottom docking can add flexible empty space above the trailing widget, but may not shrink the semantic boundary.
