...
2026-09-18 앱 금액 계약을 다시 고정했다. 공개 코드는 `OPEN` 하나만 사용하고 선택 상품에 따라 위 할인가를 반환한다. 코드를 제거하면 월간 상품은 각각 8,900원·12,900원, 연간 상품은 각각 106,800원·154,800원 정가로 돌아간다. 클라이언트 표시·결제 payload, 서버 preflight·최종 검증, PortOne 청구, 구독·주문·결제 저장과 후속 갱신은 모두 같은 확정 금액을 사용한다.
