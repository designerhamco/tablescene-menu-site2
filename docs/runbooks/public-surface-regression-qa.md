...
`만들기`의 스타터 미리보기 언어 전환과 기기 툴바·선데이 라인 비율은 별도 실제 브라우저 검사로 확인한다. 모바일 단일페이지 4종은 언어 UI가 제목 위 독립 행 오른쪽에 있는지, 태블릿 가로는 화면 회전 아이콘이 가로·세로로 왕복하고 카테고리·메뉴명·설명 계산 크기가 PC보다 작은지도 확인한다.

```bash
npm run qa:template-preview-locales
```

이 검사는 활성 템플릿 7종의 영어·중국어·일본어 21개 실제 route에서 한글 fallback, HTTP/console/page 오류, 가로 overflow와 카페 fit `ready`를 확인한다. 단일페이지는 내부 페이지명이 디자인 문구로 노출되지 않는지, PC 카테고리명이 메뉴명보다 계산 크기 1.35배 이상인지 확인한다. 이어 단일페이지 4종의 PC·태블릿·모바일 12개 화면에서 일반 상품명·텍스트칩·가격·설명과 대표 영역의 대응 역할을 비교한다. 대표 역할은 같은 `font-family`, `font-weight`, `font-style`, `letter-spacing`, `line-height` 비율, `text-transform`을 유지하면서 계산 크기만 1.25배여야 하고, 카테고리·가게 설명 및 안내는 일반 상품 설명과 동일해야 한다. 라운드 포커스 비모바일 3열 실측 폭은 1px 이내로 같아야 하며, 선데이 라인 행 간격은 기본 패딩보다 3~12px만 커야 한다. 마지막으로 선데이 라인 wrapper에서 기기 툴바가 기본으로 열리고 동일 폭을 유지한 채 위로 접히는지, 화면 회전 아이콘이 태블릿 가로·세로를 왕복하는지, 이미 `ready`인 메뉴판의 viewport를 바꾸어도 로딩 상태로 돌아가지 않고 재계산 후 `ready`·무잘림을 유지하는지 검사한다.
...
- 단일페이지 4종 × PC·태블릿·모바일: 대표 상품명·텍스트칩·가격·설명은 대응 메뉴 역할의 계산 스타일을 유지하면서 크기만 1.25배, 그 외 설명 역할 불일치 0건
- 라운드 포커스 PC·태블릿 3열 실측 폭 차이 1px 이하, 선데이 라인 PC·태블릿 행 간격 추가분 3~12px, 모카 카테고리 유동 배율 1.08 확인
