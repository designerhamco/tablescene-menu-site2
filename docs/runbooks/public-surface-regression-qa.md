...
`만들기`의 스타터 미리보기 언어 전환과 기기 툴바·선데이 라인 비율은 별도 실제 브라우저 검사로 확인한다. 모바일 단일페이지 4종은 언어 UI가 제목 위 독립 행 오른쪽에 있는지, 태블릿 가로는 기기·방향 행의 중심이 일치하고 카테고리·메뉴명·설명 계산 크기가 PC보다 작은지도 확인한다.

```bash
npm run qa:template-preview-locales
```

이 검사는 활성 템플릿 7종의 영어·중국어·일본어 21개 실제 route에서 한글 fallback, HTTP/console/page 오류, 가로 overflow와 카페 fit `ready`를 확인한다. 단일페이지는 내부 페이지명이 디자인 문구로 노출되지 않는지, 카테고리명이 메뉴명보다 계산 크기 1.35배 이상인지 확인한다. 이어 단일페이지 4종의 PC·태블릿·모바일 12개 화면에서 일반/대표 상품명, 일반/대표 텍스트칩, 일반/대표 가격, 상품 설명과 대표·카테고리·가게 설명 및 안내의 실제 계산 `font-size`, `font-family`, `font-weight`, `font-style`, `letter-spacing`, `line-height`, `text-transform`을 역할별로 비교한다. 마지막으로 선데이 라인 wrapper에서 기기 툴바가 기본으로 열리고 동일 폭을 유지한 채 위로 접히는지, 이미 `ready`인 메뉴판의 viewport를 바꾸어도 로딩 상태로 돌아가지 않고 재계산 후 `ready`·무잘림을 유지하는지 검사한다.
...
- 단일페이지 4종 × PC·태블릿·모바일: 상품명·텍스트칩·가격·설명 역할의 계산 스타일 불일치 0건
- 선데이 라인 태블릿 가로 `1180×820`: 안정화 후 타이포그래피 위계와 화면 경계 잘림 없음 육안 확인
