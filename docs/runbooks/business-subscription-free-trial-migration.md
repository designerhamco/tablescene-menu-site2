...
1. Production read-only precheck로 대상 컬럼·index가 아직 없는지 확인한다.
2. 사람의 특정 승인을 받은 뒤 migration 파일 한 건만 Production에 적용한다.
3. constraint와 partial unique index, 기존 구독 건수 불변을 확인한다.
4. Vercel Production에 `BUSINESS_SINGLE_MONTHLY_FREE_TRIAL_ENABLED=true`를 설정하고 재배포한다.
5. 신규 전용 QA 계정에서 사업자 인증과 결제수단 등록까지만 진행해 첫 결제가 발생하지 않는지 확인한다.
6. 구독의 `next_billing_at`, `current_period_end`, `trial_ends_at`이 시작 시각으로부터 정확히 30일 뒤인지 확인한다.
7. 별도 QA 구독에서 해지 예약 후에도 종료일까지 접근 가능하며 갱신 cron이 결제보다 만료를 우선하는지 확인한다.
