    ...
    <section id="services" className="relative overflow-hidden bg-[#fcfcfc] py-24">
      <div className="mx-auto max-w-7xl px-6">
        <div className="relative mb-16 text-center">
          <h2 className="type-section-title mb-6 text-zinc-900">우리의 서비스</h2>
              ...
              >
                ...
                {disabled && (
                  <div className="absolute inset-0 flex items-center justify-center bg-black/50 backdrop-blur-[2px]">
                    <span className="text-3xl font-bold uppercase tracking-widest text-white/60 drop-shadow-lg">Coming Soon</span>
                  </div>
                )}
                <div className="absolute inset-x-0 bottom-0 p-8">
                  <h3 className="type-subsection-title mb-3 flex items-center gap-3 text-white">
                    {service.title}
                    {isAi && (
                      <span className="rounded-full border border-white/20 bg-gradient-to-r from-blue-500 to-violet-600 px-2 py-0.5 text-xs font-bold leading-none text-white shadow-lg md:text-xs">
                        AI
                      </span>
                    )}
                    {!disabled && (
