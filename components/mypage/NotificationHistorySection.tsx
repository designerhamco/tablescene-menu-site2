  ...
  return (
    <section id="notification-history" className="scroll-mt-28">
      <div className="mb-5 flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div>
          <h2 className="type-section-title">알림 내역</h2>
      ...
      {items.length > 0 ? (
              ...
              >
                <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                  <div className="flex min-w-0 gap-3">
                    ...
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <h3 className="type-item-title break-keep text-zinc-950">{event.title ?? "알림"}</h3>
                        <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${isUnread ? "bg-red-50 text-red-600" : "bg-zinc-100 text-zinc-500"}`}>
                          {isUnread ? "읽지 않음" : "읽음"}
                        </span>
                      </div>
                  ...
                  <div className="shrink-0 text-left md:text-right">
                    ...
                    <p className="mt-2 text-xs font-bold text-zinc-400">{getDeliveryLabel(event)}</p>
                  </div>
                </div>
              </a>
            );
        ...
        <article className="rounded-3xl border border-dashed border-zinc-200 bg-white p-10 text-center shadow-sm">
          <h3 className="type-subsection-title">새 알림이 없습니다.</h3>
