...
test("digital time-sale countdown uses minutes and seconds below one hour", () => {
  const nowMs = Date.parse("2026-09-21T00:00:00.000Z");
  assert.equal(formatTimeSaleDigitalCountdownLabel(nowMs + 59 * 60_000 + 59_000, nowMs), "59:59");
  assert.equal(formatTimeSaleDigitalCountdownLabel(nowMs + 9_000, nowMs), "00:09");
});
...
test("digital time-sale countdown includes hours without losing fixed-width digits", () => {
  ...
  assert.equal(formatTimeSaleDigitalCountdownLabel(nowMs - 1_000, nowMs), "00:00");
});
